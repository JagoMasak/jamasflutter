...
class ServiceApi {
  static final String _baseUrl = "http://jamas.dedekj.site/api/";

  Future <ProdukPaket> listDataProdukPaket() async{
    final response = await http.get(Uri.parse(_baseUrl + "produk/jenis?JAGOMASAK-KEY=jamas123&jenis=Paket Siap Masak"));
  ...
  Future <ProdukSiap> listDataProdukSiap() async{
    final response = await http.get(Uri.parse(_baseUrl + "produk/jenis?JAGOMASAK-KEY=jamas123&jenis=Paket Siap Saji"));
    if (response.statusCode == 200){
